...
(defmethod initialize-instance :after ((self text-billboard) &key)
  (with-slots (vertex-buffer texture technique text) self
	...
	(setf technique (make-instance 'color-technique :vs-path (pathname "shaders/text-billboard.vertexshader") :fs-path (pathname "shaders/text-billboard.fragmentshader")))))
